...
@st.cache_resource
def load_artifacts():
    model = joblib.load("psychmed_model.pkl")
    drug_enc = joblib.load("drug_encoder.pkl")
    drug_list = joblib.load("drug_list.pkl")
    age_map = joblib.load("age_map.pkl")
    time_map = joblib.load("time_map.pkl")
    n_features = joblib.load("n_features.pkl")
    df = pd.read_csv("/Users/lina/Desktop/NT-WebMD/Data/df_PMEI_final.csv")
    diag_drug_map_df = pd.read_csv("/Users/lina/Desktop/NT-WebMD/Products/diagnosis_drug_map.csv")
...
# ======================
# UI layout for the tabs
# ======================

st.title("PsychMED")
st.header("Patient PMEI calculator and predictor")
st.warning("Please have in mind that this information should NOT be used as a medical advice. This is a research project and the results are not guaranteed to be accurate. The data the model trained on was collected from Web MD.")

tab1, tab2 = st.tabs([
    "Patient Profile to Medication Recommender",
    "Medication History (PMEI calulator)"
])

# ======================
# Tab 1 model (psychmed)
# ======================
...
with tab1:
    ...
    # embedding (ONLY from current symptoms)
    ...
    # FIXED MODEL VECTOR (NO HISTORY HERE)
    ...
    def recommend(patient_base, diagnosis):
        ...
        # FILTER DRUGS BY DIAGNOSIS FIRST
        allowed_drugs = diag_to_drugs.get(diagnosis, set())

        for drug in drug_list:
            # only keep drugs valid for this diagnosis
    ...
    if st.button("Generate Recommendations"):
        st.warning(
            "These recommendations are based on a research prototype model."
            "They are not clinically validated and should be interpreted with immense caution."
        )
...
# ======================
# tab 2 PMEI calculator (not for ML model)
# ======================
...
with tab2:
    ...
    for i in range(num_drugs):
        ...
        st.write("Please provide a number based on ease of taking, effectiveness and satisfaction")
        base_pmei = (st.slider("Base ratings", 2, 8, 5, key=f"base_{i}"))

        st.write("Your written review will be combined with ±2 adjustment to the base rating")
